...
icustays %>%
  ...
  labs(title = "Bar plot of first ICU type")

# Last ICU unit
table(icustays$last_careunit)

icustays %>%
  ggplot(aes(x = last_careunit)) + 
  geom_bar(aes(fill = last_careunit)) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) + 
  labs(title = "Bar plot of last ICU type")
```
There are 69619 unique `stay_id` in the `icustays` dataset and 50048 unique `subject_id` in the dataset.
