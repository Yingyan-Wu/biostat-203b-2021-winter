...
3. Perform a data quality check of the ICU stays data. Discard variables with substantial missingness, say >5000 `NA`s. Replace apparent data entry errors by `NA`s.
    ...
    #---- Load the data ----
    icustays <- 
      readRDS(paste0(repo_path, "/hw3/mimiciv_shiny", "/icu_cohort.rds")) %>%
      print(width = Inf)
...
2. Train the models using the training set.
    ...
    **Solutions:**
    ...
    # x = model.matrix(trainsubset[, 1:19], d), dat = trainsubset, alpha = 1)
...
3. Compare model prediction performance on the test set.
    ...
    **Solutions**:
    ...
    pred_logistic_outcomes <- tibble(
      probs = predict(model2, newx = x_pred, type = "response")
    ) %>%
      mutate(y_pred = ifelse(probs > 0.5, 1, 0))
    
    #---- Predicted outcome lasso ----
    x_pred_lasso <- model.matrix(
      as.formula(paste0("~ ", paste0(predictors, collapse = "+"), "- 1")), 
      testsubset)[, -1]
