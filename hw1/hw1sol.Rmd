...
5. Display the first few lines of `admissions.csv.gz`. How many rows are in this data file? How many unique patients (identified by `subject_id`) are in this data file? What are the possible values taken by each of the variable `admission_type`, `admission_location`, `insurance`, `language`, `marital_status`, and `ethnicity`? Also report the count for each unique value of these variables. (Hint: combine Linux commands `zcat`, `head`/`tail`, `awk`, `uniq`, `wc`, and so on.)
  ...
  #How many unique patients (identified by `subject_id`) are in this data file?
    zcat /usr/203b-data/mimic-iv/core/admissions.csv.gz|awk -F, 'FNR > 1 {print $1}'|
     sort -u|wc -l
...
## `admission_type`
    zcat /usr/203b-data/mimic-iv/core/admissions.csv.gz|awk -F, 'FNR > 1 {print $6}'|
      sort|uniq -c
```
      
```{bash}
##  `admission_location`
    zcat /usr/203b-data/mimic-iv/core/admissions.csv.gz|awk -F, 'FNR > 1 {print $7}'|
      sort|uniq -c
    ```

    ```{bash}      
##  `insurance`
    zcat /usr/203b-data/mimic-iv/core/admissions.csv.gz|awk -F, 'FNR > 1 {print $9}'|
      sort|uniq -c
    ```

    ```{bash} 
##  `language`
    zcat /usr/203b-data/mimic-iv/core/admissions.csv.gz|awk -F, 'FNR > 1 {print $10}'|
      sort|uniq -c
    ```

    ```{bash} 
##  `marital_status`
    zcat /usr/203b-data/mimic-iv/core/admissions.csv.gz|awk -F, 'FNR > 1 {print $11}'|
      sort|uniq -c
    ```

    ```{bash} 
## `ethnicity`
    zcat /usr/203b-data/mimic-iv/core/admissions.csv.gz|awk -F, 'FNR > 1 {print $12}'|
      sort|uniq -c
...
Used `vi` to edit the `middle.sh` file and `chmod +x` make this file executable. 
  ...
  ./middle.sh pride_and_prejudice.txt 20 5
    ```
Use the script from `middle.sh` to obtain the last five line of the first 20 lines from the text file. PENDING!
```{bash}
