...
```{r}
labevents_tble <- tbl(con, Id(schema = "public", table = "mimic_labevents_icu")) %>% print(width = Inf)
```

7. Connect to `mimic_chartevents_icu` table.
```{r}
chartevents_tble <- tbl(con, Id(schema = "public", table = "mimic_chartevents_icu")) %>% print(width = Inf)
```
...
icustays_tble %>% 
  ...
  ungroup() %>%
  # ii.  merge the admissions and patients tables
  left_join(admissions_tble, by = c("subject_id", "hadm_id")) %>%
  left_join(patients_tble, by = "subject_id") %>%
  # iii. keep adults only
  ...
  mutate("flag_die30" = case_when(is.na(day(deathtime) - day(admittime)) ~ 0,
                      (day(deathtime) - day(admittime) > 30) ~ 0,
                     (day(deathtime) - day(admittime) <= 30) ~ 1)) %>%
  # viii. save the final tibble to an `icu_cohort.rds` R data file
  # in the shiny folder
...
See the `app.R` script in the `mimiciv_shiny` folder.